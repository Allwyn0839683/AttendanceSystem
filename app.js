...
const databasename = 'attendance_system';
...
MongoClient.connect(url, function (err, db) {
  if (err) throw err;
  var dbo = db.db('attendance_system');
      ...
      dbo
        ...
        .toArray(function (err, result2) {
          ...
          for (let i = 0; i < length2; i++) {
            if (result2[i].present == 'Present') {
              total_classes_attended += 1;
            }
          }
          var percentage2 = (total_classes_attended * 100) / length2;
          percentage = (
            (percentage1 * length1 + percentage2 * length2) /
            (length1 + length2)
          ).toFixed(2);
